...
const bgColor = computed(
    () => regionColors[props.regionId % regionColors.length],
);
...
const borderStyle = computed(() => {
    ...
    const isTopBorderThick =
        rowIndex === 0 ||
        props.grid[rowIndex - 1]?.[columnIndex]?.regionId !== regionId;
    const isBottomBorderThick =
        rowIndex === boardSize - 1 ||
        props.grid[rowIndex + 1]?.[columnIndex]?.regionId !== regionId;
    const isLeftBorderThick =
        columnIndex === 0 ||
        props.grid[rowIndex]?.[columnIndex - 1]?.regionId !== regionId;
    const isRightBorderThick =
