...
export function getInvalidQueens(board: Board): Set<string> {
    ...
    for (let i = 0; i < queens.length; i++) {
        for (let j = i + 1; j < queens.length; j++) {
            ...
            // Rule 1: Same row
            if (firstQueen.rowIndex === secondQueen.rowIndex)
                isPlacementInvalid = true;
            // Rule 2: Same column
            if (firstQueen.columnIndex === secondQueen.columnIndex)
                isPlacementInvalid = true;
            // Rule 3: Same region
            if (firstQueen.regionId === secondQueen.regionId)
                isPlacementInvalid = true;
            // Rule 4: Touching diagonally or orthogonally
            if (
                Math.abs(firstQueen.rowIndex - secondQueen.rowIndex) <= 1 &&
                Math.abs(firstQueen.columnIndex - secondQueen.columnIndex) <= 1
            )
                isPlacementInvalid = true;

            if (isPlacementInvalid) {
                invalidCells.add(
                    `${firstQueen.rowIndex},${firstQueen.columnIndex}`,
                );
                invalidCells.add(
                    `${secondQueen.rowIndex},${secondQueen.columnIndex}`,
                );
            }
        }
    }
